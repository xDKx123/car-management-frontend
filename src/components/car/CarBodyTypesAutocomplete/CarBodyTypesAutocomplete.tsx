...
import './CarBodyTypesAutocomplete.css'

interface CarBodyTypesAutocompleteProps {
    handleChange: (carBodyType: string) => void
    value: string
}

const CarBodyTypesAutocomplete: FC<CarBodyTypesAutocompleteProps> = (props: CarBodyTypesAutocompleteProps) => {
    ...
    useEffect(() => {
        if (open) {
            setLoading(true)
            CarRepository.loadCarBodyTypes().then((carBodyTypes: string[] | undefined) => {
                if (carBodyTypes) {
...
export default CarBodyTypesAutocomplete
